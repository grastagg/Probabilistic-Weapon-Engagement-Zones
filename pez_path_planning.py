...
from fast_pursuer import inEngagementZone, probabalisticEngagementZoneTemp
...
def compute_spline_constraints(spl, t, pursuerPosition, pursuerPositionCov, pursuerRange, pursuerCaptureRange, pursuerSpeed, agentSpeed):
    pos = spl(t).squeeze()
    x = pos[:,0]
    y = pos[:,1]
    ...
    for i in range(len(t)):
        ...
        pez_constraint[i] = probabalisticEngagementZoneTemp(np.array([[x[i]],[y[i]]]), agentHeading, pursuerPosition,pursuerPositionCov, pursuerRange, pursuerCaptureRange, pursuerSpeed, agentSpeed)
...
def optimize_spline_path(p0, pf, v0, num_cont_points,spline_order, velocity_constraints, turn_rate_constraints, curvature_constraints, num_constraint_samples, pez_constraint_limit, pursuerPosition, pursuerPositionCov, pursuerRange, pursuerCaptureRange, pursuerSpeed, agentSpeed):
    ...
    def objfunc(xdict):
        ...
        velocity, turn_rate, curvature,pez = compute_spline_constraints(spline, t_constraints,pursuerPosition, pursuerPositionCov, pursuerRange, pursuerCaptureRange, pursuerSpeed, agentSpeed)
    ...
    optProb.addConGroup("pez", num_constraint_samples, lower=pez_constraint_limit, upper=None)
...
def plot_spline(spline, pursuerPosition, pursuerRange, pursuerCaptureRange):
    fig,ax = plt.subplots()
    t0 = spline.t[0]
    tf = spline.t[-1]
    t = np.linspace(t0, tf, 100, endpoint=True)
    velocity, turn_rate, curvature,pez = compute_spline_constraints(spline, t,pursuerPosition, pursuerPositionCov, pursuerRange, pursuerCaptureRange, pursuerSpeed, agentSpeed)
    spline_color = pez < 0.1
...
def plot_constraints(spline, velocity_limits, turn_rate_limits, curvature_limits, pez_limit):
    ...
    # axs[2].plot(t,np.ones(len(t))*curvature_limits[0],c='r')
    # axs[2].plot(t,np.ones(len(t))*curvature_limits[1],c='r')
    axs[3].set_title("Engagement Zone Probability")
...
if __name__ == '__main__':
    ...
    startingLocation = np.array([-3.0,-3.0])
    ...
    pez_constraint_limit = .5
    pursuerRange = 2
    pursuerCaptureRange = 0.1
    pursuerSpeed = 1.0
    agentSpeed = .9
    
    spline = optimize_spline_path(startingLocation, endingLocation, initialVelocity, numControlPoints,splineOrder, velocity_constraints, turn_rate_constraints, curvature_constraints, num_constraint_samples, pez_constraint_limit, pursuerPosition, pursuerPositionCov, pursuerRange, pursuerCaptureRange, pursuerSpeed, agentSpeed)
    plot_constraints(spline, velocity_constraints, turn_rate_constraints, curvature_constraints, pez_constraint_limit)
    plot_spline(spline, pursuerPosition, pursuerRange, pursuerCaptureRange)
