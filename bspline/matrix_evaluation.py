...
from bspline.helper_functions import count_number_of_control_points, find_preceding_knot_index,\
...
# def matrix_bspline_evaluation_jax(time, scale_factor, control_points, knot_points, clamped = False):
#     """
#     This function evaluates the B spline at the given time using
#     the matrix method
#     """
...
def matrix_bspline_evaluation_for_dataset(control_points, knot_points, num_points_per_interval):
    """
    ...
    for i in range(num_intervals):
        ...
        if dimension > 1:
            if i == num_intervals - 1:
                spline_data = spline_data.at[:, i * num_ppi:(i + 1) * num_ppi + 1].set(spline_data_over_interval[:, :num_ppi + 1])
            else:
                spline_data = spline_data.at[:, i * num_ppi:(i + 1) * num_ppi].set(spline_data_over_interval[:, :num_ppi])
        else:
            if i == num_intervals - 1:
                spline_data = spline_data.at[i * num_ppi:(i + 1) * num_ppi + 1].set(spline_data_over_interval[:num_ppi + 1])
            else:
    ...
    spline_data = jnp.hstack((spline_data[0,:].reshape((-1,1)),spline_data[1,:].reshape((-1,1))))

    return spline_data

@partial(jit, static_argnums=(0,4))
def matrix_bspline_derivative_evaluation_for_dataset(derivative_order, scale_factor, control_points, knot_points, num_points_per_interval):
    """
    This function evaluates the B spline for a given time data-set
    """
    # Initialize variables
    ...
    for i in range(num_intervals):
        ...
        if dimension > 1:
            if i == num_intervals - 1:
                spline_derivative_data = spline_derivative_data.at[:, i * num_ppi:(i + 1) * num_ppi + 1].set(
                    spline_derivative_data_over_interval[:, 0:num_ppi + 1])
            else:
                spline_derivative_data = spline_derivative_data.at[:, i * num_ppi:(i + 1) * num_ppi].set(
                    spline_derivative_data_over_interval[:, 0:num_ppi])
        else:
            if i == num_intervals - 1:
                spline_derivative_data = spline_derivative_data.at[i * num_ppi:(i + 1) * num_ppi + 1].set(
                    spline_derivative_data_over_interval[0:num_ppi + 1])
                ...
                spline_derivative_data = spline_derivative_data.at[i * num_ppi:(i + 1) * num_ppi].set(
                    spline_derivative_data_over_interval[0:num_ppi])
    spline_derivative_data = jnp.hstack((spline_derivative_data[0, :].reshape((-1, 1)), spline_derivative_data[1, :].reshape((-1, 1))))
...
#     K = jnp.zeros((order + 1, order + 1))
#     for i in range(order - derivative_order + 1):
#         K = K.at[i, i].set(jnp.math.factorial(order - i) / jnp.math.factorial(order - derivative_order - i))
#     K = K / scale_factor ** derivative_order
